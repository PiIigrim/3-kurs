...
class Perceptron:
    ...
    def backward(self, inputs, targets, outputs):
        if self.learning_rate == "adapt":
            ...
            self.learning_rate_output = (4 * np.sum((error ** 2 * outputs * (1 - outputs)), axis=0)) / (1 + np.sum(self.hidden_output ** 2)) * (np.sum((error * outputs * (1 - outputs)) ** 2, axis=0))

            h_error = delta_output.dot(self.weights_hidden_output.T)
            self.learning_rate_hidden = (4 * np.sum((h_error ** 2 * self.hidden_output * (1 - self.hidden_output)), axis=0)) / (1 + np.sum(self.hidden_input ** 2)) * (np.sum((h_error * self.hidden_output * (1 - self.hidden_output)) ** 2, axis=0))
    ...
    def train(self, inputs, targets, epochs, batch_size):
        for epoch in range(epochs):
            ...
            if not epoch % 100:
                print(f'Epoch {epoch + 1}, Learning rate {self.learning_rate_output}, Loss: {loss}')
...
perceptron.train(X_train, y_train, epochs=1000, batch_size=5)
